...
export default function EventCard({ event }: { event: Event }) {
  ...
  return (
    <div className="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      ...
      {/* Explicit CTA — separate from the image/title Link above, but same destination */}
      <Link
        to={`/events/${event.id}`}
        onClick={() => console.log("[EventCard] View Details clicked:", event.id)}
        className="flex items-center justify-center gap-1.5 border-t border-gray-100 py-2.5 text-sm font-medium text-blue-600 transition hover:bg-blue-50"
      >
